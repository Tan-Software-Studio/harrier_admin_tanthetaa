...
const SexualOrientationsModel = () => {

    const [SexualOrientationsData, setSexualOrientationsData] = useState([])
    ...
    const handleEdit = (id) => {
        showSingleSexualOrientations(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editSexualOrientations(fields);
        setOpenEditPopUp(false);
    }

    // const handleDelete = (id) => {
    //     deleteSexualOrientations(id);
    // }

    const SexualOrientationsColumns = [
    ...
    const getSexualOrientations = async () => {
        ...
        await axiosInstanceAuth
            .get("/v1/master/mst_sexual_orientations")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const SexualOrientationsList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    SexualOrientationsList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
                    ...
                    setSexualOrientationsData(temp)
                } else {
                    setSexualOrientationsData([])
                    setIsLoading(false);
                    toast.error(msg)
                }
            })
    ...
    const createSexualOrientations = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_sexual_orientations", {
    ...
    const showSingleSexualOrientations = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_sexual_orientations/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleSexualOrientations = JSON.parse(response);

                setFields(singleSexualOrientations)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editSexualOrientations = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_sexual_orientations/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteSexualOrientations = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_sexual_orientations/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: SexualOrientationsColumns, rows: SexualOrientationsData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD Sexual Orientations Form Pop Up */}
            ...
            {/* Edit  Sexual Orientations Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Sexual Orientation !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteSexualOrientations(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
...
export default SexualOrientationsModel;
