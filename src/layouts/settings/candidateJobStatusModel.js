...
const CandidateJobStatusModel = () => {
    ...
    const handleEdit = (id) => {
        showSingleCandidateJobStatus(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editCandidateJobStatus(fields);
        setOpenEditPopUp(false);
    }

    // const handleDelete = (id) => {
    //     deleteCandidateJobStatus(id);
    // }
    ...
    const getCandidateJobStatus = async () => {
        setIsLoading(true);
        await axiosInstanceAuth
            .get("/v1/master/mst_candidate_job_statuses")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const JobStatusData = JSON.parse(response);
    ...
    const createCandidateJobStatus = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_candidate_job_statuses", {
    ...
    const showSingleCandidateJobStatus = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_candidate_job_statuses/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleJobStatusData = JSON.parse(response);

                setFields(singleJobStatusData)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editCandidateJobStatus = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_candidate_job_statuses/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteCandidateJobStatus = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_candidate_job_statuses/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: JobStatusColumns, rows: jobStatusData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD Candidate JOb Status Form Pop Up */}
            ...
            {/* Edit Candidate JOb Status Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Candidate Job Status !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteCandidateJobStatus(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
...
export default CandidateJobStatusModel;
