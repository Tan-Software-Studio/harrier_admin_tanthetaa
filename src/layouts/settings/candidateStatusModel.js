...
const CandidatesStatusModel = () => {
    ...
    const handleEdit = (id) => {
        showSingleCandidatesStatus(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editCandidatesStatus(fields);
        setOpenEditPopUp(false);
    }

    // const handleDelete = (id) => {
    //     deleteCandidatesStatus(id);
    // }
    ...
    const getCandidatesStatus = async () => {
        setIsLoading(true);
        await axiosInstanceAuth
            .get("/v1/list/mst_candidate_statuses")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const CandidatesStatusList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    CandidatesStatusList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const createCandidatesStatus = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_candidate_statuses", {
    ...
    const showSingleCandidatesStatus = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_candidate_statuses/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleCandidatesStatus = JSON.parse(response);

                setFields(singleCandidatesStatus)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editCandidatesStatus = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_candidate_statuses/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteCandidatesStatus = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_candidate_statuses/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: CandidatesStatusColumns, rows: CandidatesStatusData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD  Candidates Status Form Pop Up */}
            ...
            {/* Edit  Candidates Status Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Candidates Status !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteCandidatesStatus(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
...
export default CandidatesStatusModel;
