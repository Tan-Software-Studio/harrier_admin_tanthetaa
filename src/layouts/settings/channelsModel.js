...
const ChannelsModel = () => {
    ...
    const getChannels = async () => {
        ...
        await axiosInstanceAuth
            .get("/v1/master/mst_channels")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const ChannelsList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    ChannelsList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const showSingleChannels = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_channels/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleChannels = JSON.parse(response);

                setFields(singleChannels)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editChannels = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_channels/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteChannels = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_channels/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
            ...
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: ChannelsColumns, rows: ChannelsData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD Channels Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteChannels(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
