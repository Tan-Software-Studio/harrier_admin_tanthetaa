...
const EmpTypeModel = () => {
    ...
    const [empTypesData, setEmpTypeData] = useState([])
    ...
    const getEmpTypes = async () => {
        ...
        await axiosInstanceAuth
            ...
            .then((res) => {
                ...
                const EmpTypeData = JSON.parse(response);
    ...
    const createEmpType = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_employer_types", {
    ...
    const showSingleEmpType = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_employer_types/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleEmpTypeData = JSON.parse(response);

                setFields(singleEmpTypeData)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editEmpType = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_employer_types/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteEmpType = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_employer_types/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: EmpTypeColumns, rows: empTypesData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD  Employer Type Form Pop Up */}
            ...
            {/* Edit  Employer Type Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Employer Type !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteEmpType(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
