...
const FaithsModel = () => {
    ...
    const [FaithsModelData, setFaithsModelData] = useState([])
    ...
    const handleEdit = (id) => {
        showSingleFaithsModel(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editFaithsModel(fields);
        setOpenEditPopUp(false);
    }

    // const handleDelete = (id) => {
    //     deleteFaithsModel(id);
    // }

    const FaithsModelColumns = [
        { Header: "title", accessor: "title", align: "left" },
        { Header: "actions", accessor: "actions", align: "center" },
    ]

    const getFaithsModel = async () => {
        setIsLoading(true);
        await axiosInstanceAuth
            .get("/v1/master/mst_faiths")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const FaithsModelList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    FaithsModelList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const showSingleFaithsModel = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_faiths/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleFaithsModel = JSON.parse(response);

                setFields(singleFaithsModel)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editFaithsModel = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_faiths/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteFaithsModel = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_faiths/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
            ...
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: FaithsModelColumns, rows: FaithsModelData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD Faiths Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteFaithsModel(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
