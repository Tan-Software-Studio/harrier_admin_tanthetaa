...
const LanguagesModel = () => {
    ...
    const handleEdit = (id) => {
        showSingleLanguage(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editLanguages(fields);
        setOpenEditPopUp(false);
    }
    ...
    const getLanguages = async () => {
        ...
        await axiosInstanceAuth
            ...
            .then((res) => {
                ...
                const LanguagesList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    LanguagesList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const showSingleLanguage = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_languages/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleLanguage = JSON.parse(response);

                setFields(singleLanguage)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editLanguages = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_languages/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteLanguages = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_languages/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
            ...
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: LanguagesColumns, rows: languagesData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD  Languages Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Language !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteLanguages(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
