...
const QualificationsModel = () => {
    ...
    const handleEdit = (id) => {
        showSingleQualification(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editQualifications(fields);
        setOpenEditPopUp(false);
    }
    ...
    const getQualifications = async () => {
        ...
        await axiosInstanceAuth
            ...
            .then((res) => {
                ...
                const QualificationsList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    QualificationsList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const createQualifications = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_qualifications", {
    ...
    const showSingleQualification = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_qualifications/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleQualification = JSON.parse(response);

                setFields(singleQualification)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editQualifications = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_qualifications/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteQualifications = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_qualifications/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: QualificationsColumns, rows: QualificationsData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD  Qualifications Form Pop Up */}
            ...
            {/* Edit  Qualifications Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Qualification !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteQualifications(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
