...
const SchoolTypesModel = () => {

    const [SchoolTypesData, setSchoolTypesData] = useState([])
    ...
    const handleEdit = (id) => {
        showSingleSchoolTypes(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editSchoolTypes(fields);
        setOpenEditPopUp(false);
    }

    // const handleDelete = (id) => {
    //     deleteSchoolTypes(id);
    // }

    const SchoolTypesColumns = [
    ...
    const getSchoolTypes = async () => {
        ...
        await axiosInstanceAuth
            .get("/v1/master/mst_school_types")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const SchoolTypesList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    SchoolTypesList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const createSchoolTypes = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_school_types", {
    ...
    const showSingleSchoolTypes = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_school_types/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleSchoolTypes = JSON.parse(response);

                setFields(singleSchoolTypes)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editSchoolTypes = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_school_types/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteSchoolTypes = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_school_types/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: SchoolTypesColumns, rows: SchoolTypesData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD Schools Types Form Pop Up */}
            ...
            {/* Edit  Schools Types Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this School Type !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteSchoolTypes(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
...
export default SchoolTypesModel;
