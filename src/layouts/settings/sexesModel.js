...
const SexesModel = () => {
    ...
    const [SexesData, setSexesData] = useState([])
    ...
    const showSingleSexes = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_sexes/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleSexes = JSON.parse(response);

                setFields(singleSexes)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editSexes = async (fields) => {
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_sexes/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteSexes = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_sexes/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
            ...
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: SexesColumns, rows: SexesData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD  Sexes Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteSexes(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
