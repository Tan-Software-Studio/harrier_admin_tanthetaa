...
const TechToolsModel = () => {

    const [TechToolsData, setTechToolsData] = useState([])
    ...
    const handleEdit = (id) => {
        showSingleTechTools(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editTechTools(fields);
        setOpenEditPopUp(false);
    }
    ...
    const TechToolsColumns = [
        { Header: "title", accessor: "title", align: "left" },
        { Header: "actions", accessor: "actions", align: "center" },
    ]

    const getTechTools = async () => {
        setIsLoading(true);
        await axiosInstanceAuth
            .get("/v1/master/mst_tech_tools")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const TechToolsList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    TechToolsList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
    ...
    const showSingleTechTools = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_tech_tools/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleTechTools = JSON.parse(response);

                setFields(singleTechTools)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editTechTools = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_tech_tools/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteTechTools = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_tech_tools/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
            ...
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: TechToolsColumns, rows: TechToolsData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD  Tech Tools Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Tech Tool !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteTechTools(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
