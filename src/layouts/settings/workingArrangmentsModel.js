...
const WorkingArrangmentsModel = () => {

    const [WorkingArrangmentsData, setWorkingArrangmentsData] = useState([])
    ...
    const handleEdit = (id) => {
        showSingleWorkingArrangments(id);
        setOpenEditPopUp(true);
    }

    const handleEditStatus = (fields) => {
        editWorkingArrangments(fields);
        setOpenEditPopUp(false);
    }

    // const handleDelete = (id) => {
    //     deleteWorkingArrangments(id);
    // }

    const WorkingArrangmentsColumns = [
        { Header: "title", accessor: "title", align: "left" },
        { Header: "actions", accessor: "actions", align: "center" },
    ]

    const getWorkingArrangments = async () => {
        setIsLoading(true);
        await axiosInstanceAuth
            .get("/v1/master/mst_working_arrangements")
            .then((res) => {
                setIsLoading(false);
                const response = Decrypt(res?.data?.data);
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
                const WorkingArrangmentsList = JSON.parse(response);

                if (res?.data?.success) {
                    const temp = []
                    WorkingArrangmentsList?.map((d, i) => {
                        temp.push({
                            title: (
                                <MDBox lineHeight={1}>
                                    <MDTypography
                    ...
                    setWorkingArrangmentsData(temp)
                } else {
                    setWorkingArrangmentsData([])
                    setIsLoading(false);
                    toast.error(msg)
                }
            })
    ...
    const createWorkingArrangments = async () => {
        ...
        await axiosInstanceAuth
            .post("/v1/master/mst_working_arrangements", {
    ...
    const showSingleWorkingArrangments = async (id) => {

        await axiosInstanceAuth
            .get(`/v1/master/mst_working_arrangements/show/${id}`)
            .then((res) => {
                const response = Decrypt(res?.data?.data);
                const singleWorkingArrangments = JSON.parse(response);

                setFields(singleWorkingArrangments)
            })
            .catch((err) => {
                console.log(err)
            })
    ...
    const editWorkingArrangments = async (fields) => {
        const encryptedData = Encrypt(
            JSON.stringify({
                title: fields?.title,
            })
        ...
        await axiosInstanceAuth
            .post(`/v1/master/mst_working_arrangements/update/${fields?.id}`, {
                response: encryptedData,
            })
            .then((res) => {
                const response = Decrypt(res?.data?.data);
    ...
    const deleteWorkingArrangments = async (id) => {
        await axiosInstanceAuth
            .delete(`/v1/master/mst_working_arrangements/delete/${id}`)
            .then((res) => {
                const msg = Decrypt(res?.data?.message).replace(/"/g, " ");
    ...
    return (
        <>
            <Card>
                ...
                <MDBox pt={3}>
                    {isLoading ? (
                        ...
                        <DataTable
                            table={{ columns: WorkingArrangmentsColumns, rows: WorkingArrangmentsData }}
                        />
                    )}
                </MDBox>
            </Card>

            {/* ADD   Working Arrangments Form Pop Up */}
            ...
            {/* Edit   Working Arrangments Form Pop Up */}
            ...
            <Modal open={isDeleteConfirmation}
                ...
                <Box sx={modalStyle}>
                    ...
                    <Typography id="modal-modal-title" variant="h6" sx={{ display: "flex", justifyContent: "center" }}>
                        you want to delete this Working Arrangment !
                    </Typography>
                    <MDBox component="form" role="form" mt={4} sx={{ display: "flex", justifyContent: "center" }}>
                        ...
                        <MDBox mx={1}>
                            <MDButton
                                onClick={(e) => deleteWorkingArrangments(deleteID)}
                                variant="outlined"
                                color="error"
                                fullWidth
                            >
...
export default WorkingArrangmentsModel;
